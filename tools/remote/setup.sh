...
custom)
	...
	[ -n "$ENDPOINT" ] || {
		echo "An endpoint is required for a custom provider." >&2
		exit 2
	}
	;;
*)
	echo "unknown provider: $PROVIDER (use buildbuddy|custom)" >&2
	exit 2
	;;
esac
...
if [ -z "$KEY" ]; then
	if [ -n "${BUILDBUDDY_API_KEY:-}" ]; then
		...
		read -r -s -p "API key for $PROVIDER (input hidden): " KEY || true
		echo
	fi
fi
[ -n "$KEY" ] || {
